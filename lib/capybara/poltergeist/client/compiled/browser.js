    ...
    if (/#/.test(url) && prevUrl.split('#')[0] === url.split('#')[0]) {
      ...
      this.currentPage.waitState('default', function() {
        if (this.statusCode === null && this.status === 'fail') {
          return command.sendError(new Poltergeist.StatusFailError(url, "Status code: " + (this.statusCode ? this.statusCode.toString() : '') + ', message: ' + (this.status ? this.status.toString() : '')));
        } else {
